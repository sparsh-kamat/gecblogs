...
function send_password_reset($email, $token)
{
    ...
    $email_template = "
            ...
            <a href= ROOT_PATH . '/app/auth/password-change.php?token=$token&email=$email'>Reset Password</a>
            ";
...
function sendemailverify($username, $email, $token)
{
    ...
    $email_template = "
        ...
        //write above using root path 
        <a href= ROOT_PATH . '/app/auth/verifyemail.php?token=$token'>Verify Email</a>
        ";
    $mail->Body = $email_template;
    $mail->send();
}
